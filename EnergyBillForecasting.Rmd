...
Steam_Data <- fread("C:/Users/paulr/Documents/R/EnergyForecasting/data/MACH Energy - Data (3).csv", colClasses = c("myDate", "integer", "Character"))
...
Utility_Data <- Electric_Data[Steam_Data, on=.(Date=Date)]
Utility_Data <- Utility_Data[,Value:= as.double(Value),] -> UD
Utility_Data[,sum(Value), month(Date)] -> ValuePerDay

ED <- as.data.frame(Electric_Data)
ED %>% filter(Date > ymd("2020-11-1")) -> ED
ggplot( aes(Date, Value)) +
  geom_point()

png(filename="bench_query_sort.png", width=600, height=1200)

ggplot(data=ED, aes(x=Date, y=Value)) + 
